...
def confirm_restoring(ag_self: ra.Agent):
    ...
    if ag_self.poll.NoRestore or mres_changed:
        ...
        usr_choice = input(
            f"{ag_self.name} value is {ag_self.in_PV.value} was {ag_self.poll.SavedVar} [ok]"
        )
        return ra.StateLogics.Idle, ""

    if motor_choice == "aSk"[1]:
        usr_choice = input(
            f"restore {ag_self.name} from {ag_self.in_PV.value} to {ag_self.poll.SavedVar}? Restore/[Ignore and skip this motor]"
        )
...
def mres_verify(ag_self: ra.Agent):
    ...
    if ag_self.poll.SavedVar is not None and ag_self.poll.ErrTol is not None:
        _new = _in
        _old = ag_self.poll.SavedVar
        _deviation = abs(_new - _old)
...
class EpicsExtendedMotor(ra.Device):
    def __init__(self, unit=None, **kwargs):
        ...
        self.user_choice_ag = ra.Agent(
            owner=self, act_on_invalid=get_user_choice, verbose=_VERBOSE_
        )
        ...
        self.dot_lvio_ag = ra.Agent(
            owner=self,
            poll_in=verify,
            act_on_invalid=confirm_restoring,
            act_on_armed=restore,
            verbose=_VERBOSE_,
        )
        self.dot_lvio_ag.poll.ErrTol = 0.5
        self.dot_lvio_ag.poll.NoRestore = True

        self.dot_movn_ag = ra.Agent(
            owner=self,
            poll_in=verify,
            act_on_invalid=confirm_restoring,
            act_on_armed=restore,
        ...
        self.dot_rrbv_ag = ra.Agent(
            owner=self,
            poll_in=verify,
            act_on_invalid=confirm_restoring,
            act_on_armed=restore_rrbv,
            verbose=_VERBOSE_,
        )
        self.dot_rrbv_ag.poll_pr = lambda ag_self: ag_self.owner.dot_off_ag.known

        self.dot_rbv_ag = ra.Agent(
            owner=self,
            poll_in=verify,
            act_on_invalid=confirm_restoring,
            act_on_armed=restore,
        ...
        def spmg_pr(ag_self: ra.Agent):
            ...
            ag_owner = ag_self.owner  # type EpicsExtendedMotor
            pr = ag_owner.dot_rbv_ag.known and ag_owner.dot_off_ag.known

            return pr

        self.dot_spmg_ag = ra.Agent(
            owner=self,
            poll_pr=spmg_pr,
            poll_in=verify,
            act_on_invalid=confirm_restoring,
        ...
        def temp(ag_self: ra.Agent):
            ...
            r = (ag_self.owner.activator_ag.known) and (
                ag_self.owner.user_choice_ag.poll.Var == "Ignored"[0]
                or ag_self.owner.dot_spmg_ag.known
            )
        ...
        def deactivate(ag_self: ra.Agent):
            if ag_self.poll.Var and ag_self.poll.Changed:
                ...
                ag_self.owner.activator_ag.poll.force(
                    False, for_cycles=-1, immediate=True
                )
                return ra.StateLogics.Done, ""

            return ag_self.act.Var, ""

        self.deactivator_ag = ra.Agent(
            owner=self, poll_in=temp, act_on_valid=deactivate, verbose=_VERBOSE_
        )
    ...
    def set_ag_pvs(self):
        ...
        if (
            ...
            for pvname in pv_names:
                agname = (
                    pvname.replace(".", "dot_", 1).replace(":", "colon_", 1).lower()
                    + "_ag"
                )
                exec_str = "self.{0}.install_pvs(inpvname='{1}', outpvname='{1}', dev_prefix='{2}', eprefix='{3}')".format(
                    agname, pvname, self.dev_prefix, self.eprefix
                )
                exec(exec_str)
    ...
    def set_saved_value(self, pvname="", value_str=""):
        ...
        agname = (
            pvname.replace(".", "dot_", 1).replace(":", "colon_", 1).lower() + "_ag"
        )
