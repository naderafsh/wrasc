...
def severity_aoa(ag_self: ra.SeverityAg):
    ...
    if ag_self.action_str.startswith(":") or ag_self.action_str.startswith("."):
        ...
        return (
            ra.StateLogics.Idle,
            ag_self.action_str + " waiting for external action...",
        )
    else:
...
def vel_poi(ag_self: ra.Agent):
    ...
    if (_moving == 0) and (_done_moving == 1):
        ...
        _actVel = (
            ag_self.owner.rbv_ag.poll.Diff / ag_self.owner.rbv_ag.poll.DiffTime
            if ag_self.owner.rbv_ag.poll.Diff
            else 0
        )
...
def tweak_val_poi(ag_self: ra.Agent):
    ...
    if ag_self.owner.vel_ag.poll.Var != 0:
        _r = ra.np.copysign(
            ag_self.owner.pv_by_name[".TWV"].value, ag_self.owner.vel_ag.poll.Var
        )
    else:
        _r = ag_self.poll.Var
    return _r, ""
...
def tweak_aov(ag_self: ra.Agent):
    ...
    if ag_self.poll.Var[0] != 0:
        ...
        ag_self.saved_value = {
            "twv": ag_self.owner.pv_by_name[".TWV"].value,
            "velo": ag_self.owner.pv_by_name[".VELO"].value,
        }
...
def pose_for_homing_poi(ag_self: ra.Agent):
    ...
    if ag_self.owner.is_rotary:
        ...
        homing_backoff_pos = (
            0 - homing_search_seconds * ag_self.owner.homing_velocity_dial.value
        )

        on_homing_position = (
            abs(ag_self.owner.rbv_ag.poll.Var - homing_backoff_pos)
            < 5 * ag_self.owner.pos_prec_ag.poll.Var
        )
    else:
        homing_backoff_pos = (
            ag_self.owner.rbv_ag.poll.Var
            + ag_self.owner.homing_direction
            * (
                ag_self.owner.pv_by_name[".HLM"].value
                - ag_self.owner.pv_by_name[".LLM"].value
            )
        )
    ...
    _r = {
        "homing_backoff_pos": homing_backoff_pos,
...
def pose_for_homing_aov(ag_self: ra.Agent):
    ...
    if target:
        ...
        if (
            target * ag_self.owner.homing_direction
            > ag_self.owner.saved_homing_soft_limit * ag_self.owner.homing_direction
        ):
            ag_self.owner.homing_soft_limit.value = (
                ag_self.owner.saved_homing_soft_limit
                + ag_self.owner.homing_direction * 10000
            )
...
class Motor(ra.Device):
    ...
    status_ref_dict = {
        ...
        ":HM_STATE": {
            7.0: ["homing_not_finished", homing_severity_30, "homing_unsuccessful", 1]
        },
        ":HM_STATUS": {
    ...
    def __init__(self, unit=None, is_rotary=False, **kwargs):
        ...
        self.master_config = {
            "msta_bit_enum": self.msta_bit_enum,
        ...
        self.status_ag = ra.Agent(
            owner=self, first_name="status_ag", poll_in=ra.status_poi, verbose=_VERBOSE_
        )

        self.fault_ag = ra.Agent(
            owner=self, first_name="fault_ag", poll_in=ra.bit_stat, verbose=_VERBOSE_
        )
        self.fault_ag.stat_pv = ":FAULT"
        self.fault_ag.ref_list = self.fault_bit_enum

        self.can_init_ag = ra.Agent(
            owner=self,
            first_name="can_init_ag",
            poll_in=can_init_poi,
            verbose=_VERBOSE_,
        )

        self.severity_ag = ra.SeverityAg(
            owner=self,
            first_name="severity_ag",
            poll_in=ra.severity_poi,
            act_on_valid=ra.severity_aov,
        ...
        self.vel_ag = ra.Agent(
            owner=self, first_name="vel_ag", poll_in=vel_poi, unit=(self.unit + "/s")
        )

        self.rbv_ag = ra.Agent(
            owner=self, first_name="rbv_ag", poll_in=rbv_poi, unit=self.unit
        )

        self.tweak_val_ag = ra.Agent(
            owner=self, first_name="tweak_ag", poll_in=tweak_val_poi, unit=self.unit
        )

        self.max_scan_vel_ag = ra.Agent(
            owner=self, first_name="max_scan_vel_ag", poll_in=max_scan_vel_poi
        )

        self.pos_prec_ag = ra.Agent(
            owner=self,
            first_name="pos_prec_ag",
            initial_value=0.001,
            unit=self.rbv_ag.unit,
        )
        # TODO hardcoded

        self.max_sync_time_ag = ra.Agent(
            owner=self, first_name="max_sync_time_ag", initial_value=0.1, unit="s"
        )
        # TODO hardcoded

        self.rbv_err_ag = ra.Agent(
            owner=self,
            first_name="rbv_err_ag",
            poll_in=rbv_err_poi,
            act_on_valid=rbv_err_aov,
            unit=self.rbv_ag.unit,
        )

        self.tweak_ag = ra.Agent(
            owner=self,
            first_name="tweak_ag"
            # , poll_in=tweak_in
        ...
        self.move_to_ag = ra.Agent(
            owner=self,
            first_name="move_to_ag",
        ...
        self.pose_for_homing_ag = ra.Agent(
            owner=self, first_name="pose_for_homing_ag", verbose=_VERBOSE_
        )

        self.pose_for_homing_ag.setup(
            poll_in=pose_for_homing_poi,
