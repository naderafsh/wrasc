...
logger_dmodel.info("SCS reactive agents module is compiling at {}".format(datetime.now()))
...
logger_default.info("SCS reactive agents module is loading at {}".format(datetime.now()))
...
default_motor_fields = ['.VAL', ':INIT.PROC', ':FORCE_UNINIT.PROC', '.RBV', '.DMOV', '.MOVN']
...
class Loci(object):
    ...
    def refresh_check(self):
        ...
        if self.position != _check_position:
            print('transform is divergent: {} -cs0> {} -to_cs> {}'.format(_check_position, _check_position_cs0,
                                                                          self.position))
...
class Device:
    ...
    def __init__(self, device_name='unnamed_device', master_config={}, device_config={}, eprefix=None, dev_prefix=None, auto_install_pvs=None, **kwargs):
...
class Agent(object):
    ...
    def setup(self,
            initial_value=None,
            # eprefix=None,
            poll_in=None, act_on_invalid=None, act_on_valid=None, act_on_armed=None,
            poll_pr=None,
            verbose=0,
            name: str = None, first_name=None, dmAgentType=None, description=None,
            time_out=None,
            unit='',
            **kwargs):
    ...
    def install_pvs(self, eprefix=None, dev_prefix=None, inpvname=None, outpvname=None, pvs_by_name=None, **kwargs):
        ...
        if inpvname:
            ...
            self.in_PV = PV(eprefix + dev_prefix + inpvname, verbose=False)  # , callback=handle_update)

        if outpvname:
            self.outpvname = outpvname
            self.out_PV = PV(eprefix + dev_prefix + outpvname, verbose=False)  # , callback=handle_update)
        ...
        if pvs_by_name:
            ...
            for _pvf in self.pvs_by_name:
                _pvfPV = PV(self.eprefix + self.dev_prefix + _pvf, verbose=False)  # , callback=handle_update)
        ...
        if (self.poll._hold_counter < 1) and (self.poll._hold_timer < timer()):
            ...
            if major_cycle:
                ...
                if _v is None:
                    ...
                    self.poll.IsStable = (self.poll.NoChangeCount > self.poll.DebounceCycles)
    ...
    def _out_proc(self, major_cycle=True):
        ...
        if major_cycle:
            ...
            if self.act.Changed:
                ...
                try:
                    self.act.Diff = _v - self.act.Var  # this may through an exception depending on dmAgentType... put this last!
                except TypeError:
                    pass
    ...
    for _ag in ddict:
        if ('dev_prefix' in ddict[_ag]) and ('pvf_list' in ddict[_ag]):
            ...
            for _pvf in _pvs_by_name:
                _pvfPV = PV(dev_prefix + _pvf, verbose=False)  # , callback=handle_update)
                pvf = _pvf
        ...
        for agname in sorted_ag_list:
            ...
            if agent.verbose > 0:
                ...
                _str = '<' + agent_var_debug_format. \
                                format(agname, abbreviated_status, return_message, in_var_str, '' if agent.unit is None else agent.unit)
                
                desc_str = '{0}({2} {3}) "{1}" "{4}"'. \
                    format(abbreviated_status, return_message, in_var_str, \
                        '' if agent.unit is None else agent.unit, agent.act.last_message)
...
def compile_dependencies(_agents_list, script_globals):
    ...
    for _this_ag in _agents_list:
        ...
        if not (pythonise_ag_name(_this_ag[0]) == _this_ag_fullname):
            print(myEsc.ERROR + 'names mismatch!!! {} vs {}'.format(pythonise_ag_name(_this_ag[0]), _this_ag_fullname), end=myEsc.END + '\n')
            exit(1)
            continue
        ...
        for method_name in method_names:
            ...
            for push_method_name in push_method_names + ['']:
                ...
                dum = [re.sub('ag_owner.', _this_ag_obj.owner_name + '.', _dum) for _dum in dum]
                #dum = [re.sub('self.', _this_ag_obj.owner_name + '.', _dum) for _dum in dum]

                dum = [re.sub('ag_self.', _this_ag_obj.name + '.', _dum) for _dum in dum]
                ...
                for _dum in dum:
                    try:
                        ...
                        print(myEsc.WARNING + '*_ag name without literal declaration (e.g. in comments) - dependency will be ignored -')

                # remove duplicates
                _depreag_names = list(set(_depreag_names))
                _depreag_list = [(_ag_name, eval(_ag_name, script_globals)) for _ag_name in _depreag_names]  # type: List[Tuple[str, Agent]]
                ...
                if len(_depreag_names) > 0:
                    print(line_str.format(err_str, dep_str,  _depreag_names, method_name), end=myEsc.END + '\n')
                else:
                    pass
    ...
    for _this_ag in _agents_list:
        ...
        if len(_ag_full_names) == 0:
            ...
            infer_str = '= ' + functools.reduce(lambda a, b:
                                                excelise_ag_name(a)+' + '+excelise_ag_name(b), _ag_full_names)
            input_str = ''
        print('{}\t{}\t{}'.format(excelise_ag_name(_this_ag_fullname), infer_str, input_str), file=f)

        print()
    f.close()

    print('\n----------------\n\n', 'Compiling dependencies pass 3: workout dependency levels', end='...\n \n')
...
def compile_n_install(initial_dict_of_agents, script_globals, eprefix=None):
    ...
    for _device in main_globals_devices:
        ...
        _device_agents = [[[_device_name, _device], _member] for _member in _members if
                          hasattr(_member[1], 'dmAgentType') and str(_member[0]).endswith('_ag')]
        device_agent_list.extend(_device_agents)

    print('{} devices found. Saving device configurations. May take a littel while... '.format(len(device_dict)))

    save_device_configs(device_dict)

    _dupls = [agent for agent in device_agent_list if not str(agent[1][0]).endswith('_ag')]  # type: List[ra.Agent]

    if _dupls != []:
        print(myEsc.ERROR, 'Duplicate agents defined: {}'.format(_dupls), end=myEsc.END + ' ')
        exit(1)
    ...
    if n != len(agents_list):
        print(myEsc.ERROR, 'WRONG number of agents {} vs {}'.format(n, len(agents_list)), end=myEsc.END + ' ')
        exit(1)
    else:
        print(myEsc.SUCCESS, ' {} agents in initial_dict_of_agents and agent_list'.format(n), end=myEsc.END + '\n-----------------\n')
    ...
    agents_sorted_by_layer = OrderedDict(sorted(initial_dict_of_agents.items(), key=lambda ag: ag[1]['layer']))  # type: OrderedDict[str, dict]
...
def save_device_configs(device_dict):
    ...
    for device_name in device_dict:
        ...
        device.device_config = {key: device.pv_by_name[key].value for key in device.pv_by_name}
...
def process_loop(agents_sorted_by_layer, n_loop=1000000, cycle_period=0.5, debug=False):
    ...
    while i < n_loop and 'RA_QUIT' not in all_ra_commands:
        ...
        _print_in, ra_commands, polls_var_list = inference(agents_sorted_by_layer, state_record, debug=debug)
        ...
        _print_out, ra_commands = action(agents_sorted_by_layer, state_record, debug=debug)
        all_ra_commands.update(ra_commands)
        process_ra_command(all_ra_commands)
        ...
        if _print_in or _print_out:
            ...
            print('end of cycle {}, {:.3f}s'.format(i, run_time), end='\n ====================================== \n')
        ...
        if pv:
            ...
            if type(list(status_ref_dict.keys())[0]) is int:
                ...
                for ref_val in status_ref_dict:
                    ...
                    inverted_logic = _stat[stat_inverted_index] if len(_stat) > stat_inverted_index else 0
    ...
    for _stat in self.owner.status_ag.poll.Var:
        if len(_stat) > stat_severity_index:
            ...
            if _stat[stat_severity_index] > _max_severity:
                ...
                _max_severity_errors = [[_stat[stat_name_index], _max_severity, reset_pv]]
            elif _stat[stat_severity_index] == _max_severity:
                _max_severity_errors.append([_stat[stat_name_index], _max_severity, reset_pv])
            else:
                _less_severity_errors.append([_stat[stat_name_index], _stat[stat_severity_index], reset_pv])
        else:
    ...
    if self.poll.Var > 0:
        ...
        if self.poll.IsStable and self.poll.Var > 0:
            ...
            if self.poll.Diff and (self.poll.Diff < 0):
                ...
                self.poll.force(for_cycles=1, forced_var=self.poll.Var-self.owner.severity_try_decrease_value)

                _fault_str = 'device severity: ' + 'has decreased to ' + self.status_record.max_severity_errors[0][0]
