...
ma_start_pos_ag.setup(
    **tst["Mot_A"],
    pass_conds=tls.assert_pos_wf(tst["Mot_A"]["L1"], Start_Pos, 10)[0],
    cry_cmds=["#{L1}jog=={Start_Pos}"],
    celeb_cmds=[],
)
ma_start_pos_ag.cry_retries = 100

# -------- motor B
Start_Pos = tst["Mot_B"]["Start_Pos"]
mb_start_pos_ag.setup(
    **tst["Mot_B"],
    pass_conds=tls.assert_pos_wf(tst["Mot_B"]["L1"], Start_Pos, 10)[0],
    cry_cmds=["#{L1}jog=={Start_Pos}"],
    celeb_cmds=[],
)
...
# mb_on_lim_ag.poll_pr = lambda ag_self: mb_start_pos_ag.is_done
#
