from examples import smargon_code_templates
import os
...
ppmac_test_IP = "10.109.25.22"
p4_root = os.path.normpath(r"C:\Users\ics\p4c\opa\mx1\ctrls\SR03BM01MCS22")
source_file_path = os.path.normpath(r"Project/PMAC Script Language/PLC Programs")
...
def stage_and_verify(
    ...
    action_msg = (
        "staged and confirmed" if not manual_learning else "staged and snapshot"
    )

    if verify_conds is None:
        return stage_name + f" -- {action_msg}."
    ...
    return f"{stage_name} -- {action_msg} and {len(verify_conds)} conditions verified."


smargon_ppmac = ppra.PPMAC(host=ppmac_test_IP, backward=True)
smargon_ppmac.connect()
assert smargon_ppmac.connected

# setp 1 : preparation of the test


def load_from_file(plc_file_name="", manual_learning=False):
    ...
    print(
        stage_and_verify(
            smargon_ppmac,
            stage_name=f"loading {plc_file_name}",
            cmds=cmds,
...
print(
    stage_and_verify(
        smargon_ppmac,
        stage_name=f"loading plc10",
        cmds=smargon_code_templates.plc10_code,
        manual_learning=False,
    )
)
...
print(
    stage_and_verify(
        smargon_ppmac,
        stage_name=f"loading prg10",
        cmds=smargon_code_templates.prog10_code,
        manual_learning=False,
    )
)
...
print(
    stage_and_verify(
        smargon_ppmac, stage_name="set_move", cmds=cmds, manual_learning=manual_learning
    )
)
